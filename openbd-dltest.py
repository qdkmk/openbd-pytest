...
def get_bib(item):
    ...
    :return: 書誌のリスト
    """
    print(requests.post(OPENBD_ENDPOINT + 'get', data={'isbn': ','.join(item)}).json())
    return requests.post(OPENBD_ENDPOINT + 'get', data={'isbn': ','.join(item)}).json()

if __name__ == '__main__':

    # ISBNのリストを10000件単位に分割
    #chunked_coverage = chunked(get_coverage(), 10000)
    result = get_bib("9784766108958")
    #print(result)
    print(json.dumps(result, indent=4))

    headers = {"content-type": "application/json"}
    url="https://api.openbd.jp/v1/get?isbn=9784766108958"
    r = requests.get(url, headers=headers)
    data = r.json()
    print(data[0]["summary"]["title"])
    # 4プロセスの並列でダウンロード
